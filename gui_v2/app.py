...
from scraper_bridge import get_bridge, _raw_to_pb_sequence
import learning
...
def api_lobby():
    ...
    """
    try:
        # 優先 1: ライブ scraper (BaccaratScraper bg thread)
        bridge = get_bridge()
        if bridge._running and bridge.scraper is not None:
            lobby_raw = bridge.get_lobby()
            if lobby_raw.get("ok"):
                # strategy_engine で分類 + スコア付け
                ...
                return jsonify({
                    ...
                    "ws_connected": lobby_raw.get("ws_connected"),
                    "tables": entries,
                })
        # 優先 2/3: DB or lobby_state.json fallback
