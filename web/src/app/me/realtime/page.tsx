...
export default async function RealtimePage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <div className="hud-label mb-2">Live</div>
        <h1 className="text-2xl sm:text-3xl font-hud">ライブ運用状況</h1>
        <p className="text-text-muted text-sm mt-2">受け子から 60 秒間隔で送信される運用データを 30 秒間隔でこのページに反映します。</p>
      </div>
      <RealtimePnlCard initial={initial} />
      <div className="p-5 rounded-2xl glass-soft border border-accent/10 text-xs text-text-muted leading-relaxed">
        <div className="font-bold text-text-muted mb-2">表示の見方</div>
