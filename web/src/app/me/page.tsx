...
export default async function MePage() {
  ...
  if (!latestOrder) { statusLabel = 'No License — Purchase to start'; statusColor = 'text-text-muted' }
  ...
  const cards: Array<{ href: string; title: string; desc: string; cta: string; tone: 'accent' | 'warn' | 'ok' | 'muted' }> = [
    ...
    { href: '/me/balance', title: '残高・チャージ', desc: `現在 $${Number(billing?.balance || 0).toFixed(2)} / 追加チャージはこちら`, cta: '管理', tone: hasActiveCharge ? 'ok' : 'warn' },
    ...
    { href: '/me/download', title: 'ダウンロード', desc: latestDeliverable?.version ? `BAFATHER v${latestDeliverable.version}` : '配布物なし', cta: '取得', tone: latestDeliverable ? 'ok' : 'muted' },
  ]
