...
function TabBar({ basePath, current }: { basePath: string; current: TabKey }) {
  return (
    <nav className="flex gap-px bg-white/[0.07] border border-white/[0.07] rounded-md mb-4 overflow-x-auto">
      {TABS.map(t => {
        ...
        return (
          <Link
            ...
            className={[
              'px-4 py-2.5 text-sm font-mono tracking-[0.1em] uppercase text-[11px] whitespace-nowrap transition flex-1 text-center',
              active
                ? 'bg-cyan/15 text-cyan border-cyan/30'
                : 'bg-surface text-text-muted hover:text-text hover:bg-white/[0.03]',
            ].join(' ')}
          >
            {t.label}
          </Link>
...
export default async function AdminUserDetailPage({
  ...
  return (
    <div>
      ...
      <TabBar basePath={`/admin/users/${id}`} current={tab} />
        ...
        <>
          <Card padded={false} className="mb-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 px-5 py-5">
              ...
              <ConfirmForm
                ...
                confirmText={billing?.suspended ? 'サスペンドを解除しますか?' : 'このユーザーをサスペンド (アクセス停止) しますか?'}
              >
          ...
          <Card padded={false}>
            <CardHead right={!!botConfig.customer_telegram_chat_id ? (
              <ConfirmForm action={unlinkTelegram} confirmText="Telegram 連携を解除しますか?">
                <Button tone="danger" size="sm" type="submit">解除</Button>
              </ConfirmForm>
            ) : undefined}>
              Bot Config / Telegram
            </CardHead>
                        ...
                        <td className="px-5 py-3">
                          {c.status === 'pending' && (
                            <div className="flex gap-1">
                              <ConfirmForm action={approveCharge} confirmText={`$${Number(c.amount)} のチャージを承認しますか?`}>
                                <input type="hidden" name="chargeId" value={c.id} />
                                <Button tone="success" size="sm" type="submit">承認</Button>
                              </ConfirmForm>
                              <ConfirmForm action={rejectCharge} confirmText="このチャージを却下しますか?">
                                <input type="hidden" name="chargeId" value={c.id} />
