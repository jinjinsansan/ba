      ...
      <nav className="border-b border-white/5 bg-bg-primary/80 backdrop-blur-xl">
        <div className="max-w-6xl mx-auto px-6 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4 text-sm">
            ...
            <Link href="/admin/tickets" className="text-white font-semibold">チケット</Link>
          </div>
        </div>
      </nav>

      <div className="max-w-6xl mx-auto px-6 py-10">
        <h1 className="text-3xl font-black mb-8">サポートチケット</h1>
        <div className="space-y-4">
            ...
            <div key={t.id} className="p-6 rounded-2xl bg-bg-card border border-white/5">
              <div className="flex items-center justify-between mb-3">
                <div>
                  ...
                  <span className="text-xs text-slate-600 ml-3">{new Date(t.created_at).toLocaleString('ja-JP')}</span>
                </div>
          ...
          {!tickets?.length && <p className="text-center text-slate-500 py-12">チケットはまだありません</p>}
        </div>
      </div>
    </div>
  )
