...
export default async function DashboardPage() {
  ...
  type DeductionRow = { date?: string; daily_profit?: number; fee_amount?: number; id?: string; note?: string }
  type InvoiceRow = { id?: string; settle_date?: string; daily_profit?: number; net_profit?: number; operator_fee_amount?: number; fee_amount?: number; outstanding_amount?: number; outstanding_fee_amount?: number; status?: string }
  type ChargeRow = { id?: string; amount?: number; status?: string; created_at?: string }
  const todayDeductionRows = (deductions || []).filter((d: DeductionRow) => String(d.date || '') === jstDate)
  const todayProfit = todayDeductionRows.reduce((s: number, d: DeductionRow) => s + Number(d.daily_profit || 0), 0)
  const todayFee = todayDeductionRows.reduce((s: number, d: DeductionRow) => s + Number(d.fee_amount || 0), 0)
  const outstandingAmount = (invoices || [])
    .filter((i: InvoiceRow) => String(i.status || '') === 'unpaid')
    .reduce((s: number, i: InvoiceRow) => s + Number(i.outstanding_amount || 0), 0)
  const pendingChargeCount = (charges || []).filter((c: ChargeRow) => String(c.status || '') !== 'confirmed').length
  const lastConfirmedCharge = (charges || []).find((c: ChargeRow) => String(c.status || '') === 'confirmed')
          ...
          {(invoices?.length || deductions?.length) ? (
            <div className="overflow-x-auto">
              <table className="min-w-[760px] w-full text-sm">
                ...
                <tbody>
                  {(invoices?.length ? invoices : deductions || []).map((row: InvoiceRow & DeductionRow, idx: number) => {
